...
class PerplexityConfig(BaseModel):
    """Perplexity API configuration for real-time research."""
...
class BotConfig(BaseSettings):
    ...
    perplexity: PerplexityConfig = Field(..., description="Perplexity configuration")
    ...
    skip_existing_positions: bool = Field(
        default=True, description="Skip betting on markets where we already have positions"
    )
    ...
    minimum_alpha_threshold: float = Field(
        default=2.0, description="DEPRECATED: Use z_threshold and enable_r_score_filtering instead"
    )

    # Risk-adjusted trading parameters (now the default system)
    z_threshold: float = Field(
        default=1.5, description="Minimum R-score (z-score) threshold for placing bets"
    )
    ...
    kelly_fraction: float = Field(
        default=0.5, ge=0.1, le=1.0, description="Fraction of Kelly to use (0.5 = half-Kelly)"
    )
    max_kelly_bet_fraction: float = Field(
        default=0.1, ge=0.01, le=0.5, description="Maximum fraction of bankroll per bet"
    )
    ...
    max_portfolio_positions: int = Field(
        default=10, description="Maximum number of positions to hold simultaneously"
    )
    ...
    hedge_ratio: float = Field(
        default=0.25, ge=0, le=0.5, description="Default hedge ratio (0.25 = hedge 25% of main bet)"
    )
    min_confidence_for_hedging: float = Field(
        default=0.6, ge=0, le=1, description="Only hedge bets with confidence below this threshold"
    )
        ...
        data.update(
            {
                ...
                # Hedging settings
                "enable_hedging": _clean_env_value(os.getenv("ENABLE_HEDGING", "true")).lower()
                == "true",
