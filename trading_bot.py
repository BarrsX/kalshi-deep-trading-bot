"""
Simple Kalshi trading bot with Perplexity research and decision making.

Uses Perplexity API for:
- Deep research with real-time web search (sonar-pro)
- Probability extraction and betting decisions (sonar-reasoning)
"""
...
from research_client import PerplexityClient
from betting_models import BettingDecision, MarketAnalysis, ProbabilityExtraction
from config import load_config
from perplexity_utils import perplexity_parse_pydantic
...
class SimpleTradingBot:
    ...
    async def initialize(self):
        ...
        self.research_client = PerplexityClient(self.config.perplexity)

        # Test connections
        await self.kalshi_client.login()
        self.console.print("[green]✓ Kalshi API connected[/green]")
        self.console.print("[green]✓ Perplexity API ready (research + analysis)[/green]")
        ...
        self.console.print(
            f"[blue]Max events to analyze: {self.config.max_events_to_analyze}[/blue]"
        )
        self.console.print(f"[blue]Research batch size: {self.config.research_batch_size}[/blue]")
        self.console.print(
            f"[blue]Skip existing positions: {self.config.skip_existing_positions}[/blue]"
        )
        self.console.print(
            f"[blue]Minimum time to event strike: {self.config.minimum_time_remaining_hours} hours (for events with strike_date)[/blue]"
        )
        self.console.print(
            f"[blue]Max markets per event: {self.config.max_markets_per_event}[/blue]"
        )
        ...
        self.console.print(
            f"[blue]Risk hedging: {hedging_status} (ratio: {self.config.hedge_ratio}, min confidence: {self.config.min_confidence_for_hedging})[/blue]"
        )

        # Show risk-adjusted trading settings
        self.console.print(
            f"[blue]R-score filtering: Enabled (z-threshold: {self.config.z_threshold})[/blue]"
        )
        if self.config.enable_kelly_sizing:
            self.console.print(
                f"[blue]Kelly sizing: Enabled (fraction: {self.config.kelly_fraction}, bankroll: ${self.config.bankroll})[/blue]"
            )
        self.console.print(
            f"[blue]Portfolio selection: {self.config.portfolio_selection_method} (max positions: {self.config.max_portfolio_positions})[/blue]\n"
        )
    ...
    def calculate_risk_adjusted_metrics(
        self, research_prob: float, market_price: float, action: str
    ) -> dict:
        """
    ...
    def apply_portfolio_selection(
        self, analysis: MarketAnalysis, event_ticker: str
    ) -> MarketAnalysis:
        """
        ...
        if self.config.portfolio_selection_method == "top_r_scores":
            ...
            if rejected_decisions:
                logger.info(
                    f"Portfolio selection: kept top {len(selected_decisions)} positions, "
    ...
    async def get_top_events(self) -> List[Dict[str, Any]]:
        ...
        with Progress(
            ...
            try:
                ...
                self.console.print(
                    f"[blue]• Fetched {len(events)} events (will filter to top {self.config.max_events_to_analyze} after position filtering)[/blue]"
                )
                ...
                for event in events[:10]:
                    ...
                    table.add_row(
                        ...
                        event.get("title", "N/A")[:35]
                        + ("..." if len(event.get("title", "")) > 35 else ""),
    ...
    async def get_markets_for_events(
        ...
        for event in events:
            ...
            if markets:
                ...
                if total_markets > len(markets):
                    self.console.print(
                        f"[green]✓ Using top {len(markets)} markets for {event_ticker} (from {total_markets} total)[/green]"
                    )
                else:
                    self.console.print(
        ...
        self.console.print(
            f"[green]✓ Processing {total_markets} total markets across {len(event_markets)} events[/green]"
        )
    ...
    async def filter_markets_by_positions(
        ...
        with Progress(
            ...
            for event_ticker, data in event_markets.items():
                ...
                if event_has_positions:
                    ...
                    self.console.print(
                        f"[yellow]⚠ Skipping entire event {event_ticker}: Has existing positions[/yellow]"
                    )
                else:
                    # No positions found, keep the entire event
                    filtered_event_markets[event_ticker] = {"event": event, "markets": markets}
                    total_markets_after += len(markets)
                    self.console.print(
                        f"[green]✓ Keeping entire event {event_ticker}: No existing positions[/green]"
                    )
        ...
        if len(filtered_event_markets) == 0:
            ...
            self.console.print(
                f"[green]✓ Estimated time saved by skipping research: ~{time_saved_estimate} minutes[/green]"
            )
    ...
    def _parse_probabilities_from_research(
        ...
        """Parse probability predictions from Perplexity research text."""
        probabilities = {}
    ...
    async def research_events(self, event_markets: Dict[str, Dict[str, Any]]) -> Dict[str, str]:
        """Research each event and its markets using Perplexity real-time search."""
        ...
        with Progress(
            ...
            for i in range(0, len(event_items), batch_size):
                ...
                self.console.print(
                    f"[blue]Processing research batch {i//batch_size + 1} with {len(batch)} events[/blue]"
                )
                ...
                for event_ticker, data in batch:
                    ...
                    if event and markets:
                        ...
                        tasks.append(
                            asyncio.wait_for(coro, timeout=self.config.research_timeout_seconds)
                        )
                    else:
                ...
                try:
                    ...
                    for (event_ticker, data), result in zip(batch, results):
                        if not isinstance(result, Exception) and result:
                            ...
                            self.console.print(
                                f"[red]✗ Failed to research {event_ticker}: {err}[/red]"
                            )
                            progress.update(task, advance=1)
    ...
    def _fix_semantic_inversions(
        self, extraction: ProbabilityExtraction, markets: List[Dict[str, Any]]
    ) -> ProbabilityExtraction:
        """
        Detect and fix semantic inversions where the reasoning discusses the opposite
        direction of what the market is asking about.

        For example:
        - Market asks "below $70k?" but reasoning says "85% chance stays above" → should be 15%
        - Market asks "above $100k?" but reasoning says "90% unlikely to reach" → should be 10%
        """
        # Keywords indicating the market is asking about downside/below scenarios
        below_keywords = [
            "below",
            "under",
            "low",
        ...
        # Keywords indicating the market is asking about upside/above scenarios
        above_keywords = [
            "above",
            "over",
            "high",
        ...
        # Keywords in reasoning that indicate discussing the OPPOSITE direction
        stays_above_patterns = [
            r"stays?\s+above",
            r"remain\w*\s+above",
            r"hold\w*\s+above",
            r"support\s+at",
            r"won\'?t\s+(go|drop|fall)\s+below",
            r"unlikely\s+to\s+(drop|fall|go)\s+below",
            r"chance\s+(it\s+)?stays?\s+above",
            r"probability\s+(it\s+)?stays?\s+above",
            r"will\s+(likely\s+)?stay\s+above",
            r"expected\s+to\s+stay\s+above",
        ]
        stays_below_patterns = [
            r"stays?\s+below",
            r"remain\w*\s+below",
            r"hold\w*\s+below",
            r"resistance\s+at",
            r"won\'?t\s+(go|rise|reach)\s+above",
            r"unlikely\s+to\s+(rise|reach|go)\s+above",
            r"chance\s+(it\s+)?stays?\s+below",
            r"probability\s+(it\s+)?stays?\s+below",
            r"will\s+(likely\s+)?stay\s+below",
            r"expected\s+to\s+stay\s+below",
        ]

        for market_prob in extraction.markets:
            # Find the market title
            market_title = market_prob.title.lower() if market_prob.title else ""
            reasoning = market_prob.reasoning.lower() if market_prob.reasoning else ""

            # Also check the original market data for title
            for m in markets:
                if m.get("ticker") == market_prob.ticker:
                    market_title = m.get("title", "").lower()
                    break

            # Determine what direction the market is asking about
            market_asks_below = any(kw in market_title for kw in below_keywords)
            market_asks_above = any(kw in market_title for kw in above_keywords)

            # Check if reasoning discusses the opposite direction
            reasoning_discusses_staying_above = any(
                re.search(p, reasoning) for p in stays_above_patterns
            )
            reasoning_discusses_staying_below = any(
                re.search(p, reasoning) for p in stays_below_patterns
            )

            should_invert = False
            inversion_reason = ""

            # Case 1: Market asks about going BELOW, but reasoning talks about staying ABOVE
            if (
                market_asks_below
                and reasoning_discusses_staying_above
                and not reasoning_discusses_staying_below
            ):
                should_invert = True
                inversion_reason = "Market asks 'below' but reasoning discusses 'stays above'"

            # Case 2: Market asks about going ABOVE, but reasoning talks about staying BELOW
            elif (
                market_asks_above
                and reasoning_discusses_staying_below
                and not reasoning_discusses_staying_above
            ):
                should_invert = True
                inversion_reason = "Market asks 'above' but reasoning discusses 'stays below'"

            # Additional check: if probability is very high (>80%) and market asks about an extreme scenario
            # with reasoning suggesting the opposite, likely needs inversion
            if not should_invert and market_prob.research_probability > 80:
                # Check for patterns like "X% chance above Y" when market asks "below Y"
                prob_above_pattern = (
                    r"(\d+)%?\s*(chance|probability)?\s*(it\s+)?(stays?|remains?|holds?)?\s*above"
                )
                prob_below_pattern = (
                    r"(\d+)%?\s*(chance|probability)?\s*(it\s+)?(stays?|remains?|holds?)?\s*below"
                )

                if market_asks_below and re.search(prob_above_pattern, reasoning):
                    should_invert = True
                    inversion_reason = "High probability with 'above' language for 'below' market"
                elif market_asks_above and re.search(prob_below_pattern, reasoning):
                    should_invert = True
                    inversion_reason = "High probability with 'below' language for 'above' market"

            if should_invert:
                old_prob = market_prob.research_probability
                new_prob = 100.0 - old_prob
                logger.warning(
                    f"SEMANTIC INVERSION DETECTED for {market_prob.ticker}: "
                    f"{inversion_reason}. Correcting {old_prob:.1f}% → {new_prob:.1f}%"
                )
                self.console.print(
                    f"[yellow]⚠ Semantic inversion fix for {market_prob.ticker}: "
                    f"{old_prob:.1f}% → {new_prob:.1f}% ({inversion_reason})[/yellow]"
                )
                market_prob.research_probability = new_prob
                market_prob.reasoning = f"{market_prob.reasoning} | [INVERTED: {inversion_reason}] Original reasoning discussed opposite direction. Corrected probability."

        return extraction

    async def _extract_probabilities_for_event(
        self, event_ticker: str, research_text: str, event_markets: Dict[str, Dict[str, Any]]
        ...
        try:
            ...
            ⚠️ CRITICAL - CHECK FOR SEMANTIC INVERSIONS:
            Before finalizing each probability, verify the research reasoning matches what the market is asking:
            
            - If the market asks "Will X go BELOW $Y?" (or "yearly low below $Y", "drop to $Y", etc.):
              → The probability should reflect the chance of going BELOW, not staying ABOVE
              → If research says "97% chance it stays above $Y", the correct answer is 3% (inverted)
              
            - If the market asks "Will X go ABOVE $Y?" (or "yearly high above $Y", "rise to $Y", etc.):
              → The probability should reflect the chance of going ABOVE, not staying BELOW
              
            - Look for keywords in market titles: "below", "under", "drop", "low", "floor" = asking about downside
            - Look for keywords in market titles: "above", "over", "rise", "high", "ceiling" = asking about upside
            
            EXAMPLE OF COMMON ERROR TO AVOID:
            Market: "BTC yearly low below $60k?"
            Research says: "Strong support, 97% probability BTC stays above $60k"
            ❌ WRONG: Reporting 97% (this is the inverse!)
            ✅ CORRECT: Reporting 3% (the market asks about going BELOW, not staying above)
            
            Double-check each probability: Does your reasoning MATCH the direction the market is asking about?
            """

            # Use Perplexity for structured output parsing
            extraction = await perplexity_parse_pydantic(
                self.config.perplexity,
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional prediction market analyst. Extract probability estimates from research with structured output.",
                    },
            ...
            # Post-process: detect and fix semantic inversions
            extraction = self._fix_semantic_inversions(extraction, markets)
    ...
    async def extract_probabilities(
        self, research_results: Dict[str, str], event_markets: Dict[str, Dict[str, Any]]
    ) -> Dict[str, ProbabilityExtraction]:
        """Extract structured probabilities from research results using Perplexity in parallel."""
        ...
        with Progress(
            ...
            for result in results:
                ...
                if extraction is not None:
                    ...
                    self.console.print(
                        f"[green]✓ Extracted probabilities for {event_ticker}[/green]"
                    )
                    ...
                    for market_prob in extraction.markets:
                        self.console.print(
                            f"  {market_prob.ticker}: {market_prob.research_probability:.1f}%"
                        )
                else:
                    self.console.print(
                        f"[red]✗ Failed to extract probabilities for {event_ticker}[/red]"
                    )

                progress.update(task, advance=1)

        self.console.print(
            f"[green]✓ Extracted probabilities for {len(probability_extractions)} events[/green]"
        )
    ...
    async def get_betting_decisions(
        ...
        """Use Perplexity to make structured betting decisions per event in parallel."""
        ...
        with Progress(
            ...
            for result in results:
                ...
                if event_analysis is not None:
                    ...
                    self.console.print(
                        f"[green]✓ Generated {len(event_analysis.decisions)} decisions for {event_ticker}[/green]"
                    )
                else:
                    self.console.print(
                        f"[red]✗ Failed to generate decisions for {event_ticker}[/red]"
                    )
        ...
        if hedge_decisions:
            ...
            self.console.print(
                f"[blue]💡 Generated {len(hedge_decisions)} hedge bets (${hedge_bet_total:.2f}) for risk management[/blue]"
            )
        ...
        analysis = MarketAnalysis(
            ...
            summary=f"Analyzed {len(processable_events)} events. "
    ...
    def _generate_hedge_decisions(
        ...
        for main_decision in main_decisions:
            ...
            hedge_amount = min(
                main_decision.amount * self.config.hedge_ratio, self.config.max_hedge_amount
            )
    ...
    def _display_event_decisions(self, event_ticker: str, event_analysis: MarketAnalysis):
        ...
        actionable_decisions = [
            decision for decision in event_analysis.decisions if decision.action != "skip"
        ]

        # Check if any decisions were adjusted due to mutually exclusive constraint
        mutually_exclusive_adjustments = [
            decision
            for decision in event_analysis.decisions
        ...
        if mutually_exclusive_adjustments:
            self.console.print(
                f"[blue]ℹ Strategic hedge betting: {len(mutually_exclusive_adjustments)} positions sized for mutually exclusive event[/blue]"
            )

        # Show strategic filtering info if applicable
        if strategic_filtering_skips:
            self.console.print(
                f"[yellow]⚡ Strategic filtering: {len(strategic_filtering_skips)} lower-value opportunities skipped, focused on best positions[/yellow]"
            )

        # Show event summary
        if event_analysis.total_recommended_bet > 0:
            self.console.print(
                f"[blue]Event total: ${event_analysis.total_recommended_bet:.2f} | High confidence: {event_analysis.high_confidence_bets}[/blue]"
            )
    ...
    async def _get_event_betting_decisions(
        ...
        for market in markets:
            ...
            if ticker in market_odds:
                ...
                if yes_mid_price is None or no_mid_price is None:
                    logger.warning(
                        f"Market {ticker}: Missing bid/ask data - yes_bid={yes_bid}, yes_ask={yes_ask}, no_bid={no_bid}, no_ask={no_ask}"
                    )
        ...
        for md in markets_with_odds:
            ...
            if yes_mid in (None, 0) or no_mid in (None, 0):
                logger.info(
                    f"Skipping market {md.get('ticker', '')}: missing yes/no price (yes_mid={yes_mid}, no_mid={no_mid})"
                )
                continue
        ...
        try:
            # Use Perplexity for structured output parsing
            analysis = await perplexity_parse_pydantic(
                self.config.perplexity,
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional prediction market trader.",
                    },
                    {"role": "user", "content": prompt},
                ],
            ...
            analysis = self._add_human_readable_names(analysis, event_info, markets, market_odds)

            # Apply alpha threshold validation to ensure minimum edge requirements
            analysis = self._apply_alpha_threshold_validation(
                analysis, event_ticker, markets, probability_extraction, market_odds
            )
    ...
    def _add_human_readable_names(
        ...
        """Add human-readable names to betting decisions using market_odds data."""
        # Create lookup for market names from the basic market list
        ...
        for decision in analysis.decisions:
            ...
            # Try to get a more descriptive market name from market_odds
            ticker = decision.ticker
            if ticker in market_odds:
                odds_data = market_odds[ticker]
                # Prefer subtitle or yes_sub_title for more specific naming
                # For bracket markets like "BTC below $70k", yes_sub_title often has the bracket value
                subtitle = odds_data.get("subtitle", "")
                yes_sub_title = odds_data.get("yes_sub_title", "")
                title = odds_data.get("title", "")

                # Build a descriptive name
                if subtitle:
                    # Subtitle often contains the specific bracket/option (e.g., "$70,000 or higher")
                    decision.market_name = subtitle
                elif yes_sub_title:
                    # yes_sub_title describes what YES means (e.g., "Below $70k")
                    decision.market_name = yes_sub_title
                elif title:
                    decision.market_name = title
                else:
                    decision.market_name = market_names.get(ticker, ticker)
            else:
                decision.market_name = market_names.get(ticker, ticker)
    ...
    def _enforce_mutually_exclusive_constraint(
        self, analysis: MarketAnalysis, event_ticker: str
    ) -> MarketAnalysis:
        ...
        logger.warning(
            f"Event {event_ticker}: Multiple YES bets found in mutually exclusive event. Keeping only highest confidence bet."
        )
        ...
        analysis.high_confidence_bets = len(
            [d for d in filtered_decisions if d.action != "skip" and d.confidence > 0.7]
        )

        return analysis

    def _apply_alpha_threshold_validation(
        self,
        analysis: MarketAnalysis,
        event_ticker: str,
        markets: List[Dict[str, Any]],
        ...
        for decision in analysis.decisions:
            ...
            if decision.ticker in market_odds:
                ...
                if decision.action == "buy_yes":
                    market_odds_data = (
                        ticker_odds.get("yes_ask", 0) / 100.0
                    )  # Convert to probability
                elif decision.action == "buy_no":
                    market_odds_data = (
                        ticker_odds.get("no_ask", 0) / 100.0
                    )  # Convert to probability
        ...
        # Recalculate totals
        analysis.total_recommended_bet = sum(
            d.amount for d in validated_decisions if d.action != "skip"
        )
        analysis.high_confidence_bets = len(
            [d for d in validated_decisions if d.action != "skip" and d.confidence > 0.7]
        )

        return analysis

    def _apply_strategic_filtering(
        self, analysis: MarketAnalysis, event_ticker: str
    ) -> MarketAnalysis:
        """Apply strategic filtering to ensure selective betting."""
        filtered_decisions = []
        ...
        # Recalculate totals
        analysis.total_recommended_bet = sum(
            d.amount for d in filtered_decisions if d.action != "skip"
        )
        analysis.high_confidence_bets = len(
            [d for d in filtered_decisions if d.action != "skip" and d.confidence > 0.7]
        )
    ...
    async def place_bets(
        ...
        for decision in actionable_decisions:
            if self.config.dry_run:
                self.console.print(
                    f"[blue]DRY RUN: Would place {decision.action} bet of ${decision.amount} on {decision.ticker}[/blue]"
                )
            else:
                ...
                if result.get("success"):
                    self.console.print(
                        f"[green]✓ Placed {decision.action} bet of ${decision.amount} on {decision.ticker}[/green]"
                    )
                else:
                    self.console.print(
                        f"[red]✗ Failed to place bet on {decision.ticker}: {result.get('error', 'Unknown error')}[/red]"
                    )
    ...
    def _sanitize_for_csv(self, text: str, max_length: int = 500) -> str:
        """
        Sanitize text for CSV output by removing problematic characters and truncating.
    ...
    def save_betting_decisions_to_csv(
        ...
        for decision in analysis.decisions:
            ...
            csv_row = {
                ...
                "event_title": self._sanitize_for_csv(event_title, max_length=100),
                "market_ticker": decision.ticker,
                "market_title": self._sanitize_for_csv(market_title, max_length=100),
                # Decision details
                "action": decision.action,
                "bet_amount": decision.amount,
                "confidence": decision.confidence,
                "reasoning": self._sanitize_for_csv(decision.reasoning, max_length=300),
                # Research data (original scale 0-100%)
                "research_probability": research_probability,
                "research_reasoning": self._sanitize_for_csv(research_reasoning, max_length=300),
                ...
                # Research context (truncated for CSV readability)
                "research_summary": self._sanitize_for_csv(research_summary, max_length=500),
                "raw_research": self._sanitize_for_csv(raw_research, max_length=1000),
            }
            ...
            if decision.ticker in market_odds:
                ...
                # Title-related fields first - use empty string instead of None to prevent CSV issues
                market_enriched["market_title_full"] = m.get("title") or ""
                market_enriched["market_subtitle"] = m.get("subtitle") or ""
                market_enriched["market_yes_sub_title"] = m.get("yes_sub_title") or ""
                market_enriched["market_no_sub_title"] = m.get("no_sub_title") or ""

                # Remaining attributes (aligned to Kalshi market schema when present)
                market_enriched.update(
                    {
                        "market_event_ticker": m.get("event_ticker") or "",
                        "market_market_type": m.get("market_type") or "",
                        "market_open_time": m.get("open_time") or "",
                        "market_close_time": m.get("close_time") or "",
                        "market_expiration_time": m.get("expiration_time") or "",
                        "market_latest_expiration_time": m.get("latest_expiration_time") or "",
                        "market_settlement_timer_seconds": m.get("settlement_timer_seconds"),
                        "market_status": m.get("status") or "",
                        "market_response_price_units": m.get("response_price_units") or "",
                        ...
                        "market_result": m.get("result") or "",
                        "market_can_close_early": m.get("can_close_early"),
                        "market_expiration_value": m.get("expiration_value") or "",
                        "market_category": m.get("category") or "",
                        "market_risk_limit_cents": m.get("risk_limit_cents"),
                        "market_rules_primary": self._sanitize_for_csv(
                            m.get("rules_primary") or "", max_length=200
                        ),
                        "market_rules_secondary": self._sanitize_for_csv(
                            m.get("rules_secondary") or "", max_length=200
                        ),
        ...
        if csv_data:
            fieldnames = [
                ...
                "market_ticker",
                "market_title",
                # Market subtitle fields (human-readable option names)
                "market_subtitle",
            ...
            # Append additional market attributes (subtitle fields already in base fieldnames above)
            ...
            # Still create the CSV with headers only so user knows the process ran
            base_fieldnames = [
                "timestamp",
                "event_ticker",
                "event_title",
            ...
            logger.warning("No betting decisions to save - created empty CSV with headers")
            self.console.print(
                f"[yellow]No betting decisions to save - empty CSV created: {filepath}[/yellow]"
            )
    ...
    async def run(self):
        ...
        try:
            ...
            if len(event_markets) > self.config.max_events_to_analyze:
                ...
                self.console.print(
                    f"[blue]• Limited to top {len(event_markets)} events by volume after position filtering[/blue]"
                )
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="Simple Kalshi trading bot with Perplexity AI research and decision making",
...
Configuration:
  ...
  Trading modes:
    ...
    parser.add_argument(
        "--live", action="store_true", help="Enable live trading (default: dry run mode)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--version", action="version", version="Kalshi Trading Bot 1.0.0")
