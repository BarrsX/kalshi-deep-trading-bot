...
class KalshiClient:
    ...
    async def get_events(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for event in all_events:
                ...
                # Always filter out markets that have already closed (close_time < now)
                # Also optionally filter by max_close_ts if provided
                filtered_markets = []
                for market in all_markets:
                    ...
                    try:
                        ...
                        # If max_close_ts filter is enabled, also check upper bound
                        if self.max_close_ts is not None and close_ts > self.max_close_ts:
                            continue
    ...
    async def get_markets_for_event(self, event_ticker: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Always filter out markets that have already closed
            # Also apply max_close_ts filter if set
            now = datetime.now(timezone.utc)
            current_ts = int(now.timestamp())
            filtered_markets = []
            for market in all_markets:
    ...
    async def get_market_with_odds(self, ticker: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self.client.get(f"/trade-api/v2/markets/{ticker}", headers=headers)
            response.raise_for_status()
            ...
            # Return all useful fields including human-readable names
            return {
                "ticker": market.get("ticker", ""),
                "title": market.get("title", ""),
                "subtitle": market.get("subtitle", ""),
                "yes_sub_title": market.get("yes_sub_title", ""),  # Human-readable YES option name
                "no_sub_title": market.get("no_sub_title", ""),  # Human-readable NO option name
                "yes_bid": yes_bid,
                "no_bid": no_bid,
                "yes_ask": yes_ask,
                "no_ask": no_ask,
    ...
    async def get_user_positions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self.client.get("/trade-api/v2/portfolio/positions", headers=headers)
            response.raise_for_status()
    ...
    async def place_order(self, ticker: str, side: str, amount: float) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Order placed: {ticker} {side} ${amount} (max cost: {buy_max_cost_cents} cents)"
            )
            return {
                "success": True,
